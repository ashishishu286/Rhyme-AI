...
## Goal  
Introduce quantitative measurement by implementing token-based **rhyme density** calculation.

## What I Built  
- Refactored line processing to preserve **word occurrences (duplicates)**  
- Replaced type-based storage (`lineDict[word]`) with token-based `instances` list  
- Compared word pairs using token indices:
  - `for i in range(n-1)`
  - `for j in range(i+1)`  
- Integrated `pair_count` directly inside detection loop (`score >= 4`)  
- Implemented density formula:

total_possible_pairs = n * (n - 1) / 2

density = pair_count / total_possible_pairs


- Ensured numerator and denominator operate on the same token universe  
...
Input:  
"Chin checks chinchilla till it was killer, chin shin thin tin"

Output (conceptually):  
- `n = total valid word occurrences`
- `pair_count = number of rhyming token pairs`
- `density = pair_count / (nC2)`

---

## Key Concepts Learned  
- Importance of keeping numerator and denominator mathematically consistent  
- Difference between type-based and token-based modeling  
- Why duplicate collapsing breaks combinatorics  
- Density must reflect actual pair comparisons, not reconstructed counts  
- Clean separation between detection logic and quantitative metrics  

---

## Problems Faced  
- Conflict between unique-word clustering and duplicate-sensitive density  
- Reverse-calculating pair counts from cluster data caused distortion  
- Misalignment between word count and comparison universe  
- Weak rhyme inclusion affecting density accuracy  

---

## Solutions Implemented  
- Switched to token-based instance modeling  
- Counted rhyme pairs directly during comparison  
- Removed post-processing adjustments to `pair_count`  
- Standardized density threshold to `score >= 4`  
- Ensured no recomputation from cluster structure  
...
- Duplicate-aware rhyme detection  
- Token-level rhyme density calculation  
- Mathematically consistent pair modeling  
- Clean quantitative foundation for future flow metrics  

Project now includes:
- Exact rhyme detection  
- Sliding-window similarity scoring  
- Stress-aware weighted scoring  
- Rhyme strength classification  
- Token-based rhyme density metric  

This completes the transition from qualitative rhyme detection to measurable rhyme complexity.